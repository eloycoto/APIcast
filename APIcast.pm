...
sub lock {
    my ($fh) = @_;
    flock($fh, LOCK_EX) or bail_out "cannot get prove lock.";
    # and, in case someone appended while we were waiting...
    seek($fh, 0, SEEK_END) or bail_out "cannot get prove lock.";
}

sub unlock {
    my ($fh) = @_;
    flock($fh, LOCK_UN) or bail_out "Cannot release prove lock";
}

sub get_random_port {
    my $tries = 1000;
    my $ServerPort;
    lock($prove_filename_lock);
